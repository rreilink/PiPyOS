...
2018, Rob Reilink: 
  - fixed: hang for transfers with read > ~30 bytes
  - enable interrupts on lld_init
  - support multiple I2C peripherals
...
static void serve_interrupt(void *dummy) {
  (void) dummy;
  
  I2CDriver *drivers[NUM_I2C_PERIPHERALS] = {&I2C0, &I2C1};

  // All I2C peripherals share one interrupt. Iterate over all peripherals to
  // handle their status
  for (int i = 0;i<NUM_I2C_PERIPHERALS; i++) {
    
    I2CDriver *i2cp = drivers[i];
    
    if (i2cp->state == I2C_UNINIT || i2cp->state == I2C_STOP) {
        continue; // short-cut: skip this peripheral since it is not active
    }
    ...
    if (status & (BSC_CLKT | BSC_ERR)) {
        ...
        // Clear error flags and done bit
        device->status = BSC_CLKT | BSC_ERR | BSC_DONE;
    }
...
void i2c_lld_init(void) {
  ...
  I2C0.scl_pin = GPIO0_PAD;
  I2C0.sda_pin = GPIO1_PAD;
  I2C0.alt_function = GPFN_ALT0;
  i2cObjectInit(&I2C0);

  I2C1.device = BSC1_ADDR;
  I2C1.scl_pin = GPIO2_PAD;
  I2C1.sda_pin = GPIO3_PAD;
  I2C1.alt_function = GPFN_ALT0;
  i2cObjectInit(&I2C1);
}
...
void i2c_lld_start(I2CDriver *i2cp) {
  ...
  bcm2835_gpio_fnsel(i2cp->scl_pin, i2cp->alt_function);
  bcm2835_gpio_fnsel(i2cp->sda_pin, i2cp->alt_function);
  ...
  hal_register_interrupt(53, serve_interrupt, NULL);
  IRQ_ENABLE2 = (1<<(53-32));  
}
...
void i2c_lld_stop(I2CDriver *i2cp) {
  ...
  bcm2835_gpio_fnsel(i2cp->scl_pin, GPFN_IN);
  bcm2835_gpio_fnsel(i2cp->sda_pin, GPFN_IN);
